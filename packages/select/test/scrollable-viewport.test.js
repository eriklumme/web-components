...
describe('scrollable viewport', () => {
  let scrollableContainer, container, select, overlay, button, inputField;
  ...
  beforeEach(() => {
    ...
    inputField = select.shadowRoot.querySelector('[part~="input-field"]');
  ...
  it('should update the position on scrolling', () => {
    ...
    expect(overlay.getBoundingClientRect().bottom).to.be.closeTo(inputField.getBoundingClientRect().bottom, 1);
    expect(overlay.getBoundingClientRect().left).to.be.closeTo(inputField.getBoundingClientRect().left, 1);
  ...
  it('should update the position on iron-resize event', () => {
    ...
    expect(overlay.getBoundingClientRect().top).to.be.closeTo(inputField.getBoundingClientRect().top, 1);
    expect(overlay.getBoundingClientRect().left).to.be.closeTo(inputField.getBoundingClientRect().left, 1);
  });
});
