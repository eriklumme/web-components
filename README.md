...
As long as your application uses webpack, you can modify the webpack config to resolve the web
components modules from your local clone / checkout instead of the versions that were installed by the application.
...
- running Jetty integration tests from the [Flow components repository](https://github.com/vaadin/flow-components)
  - running the tests will generate a `webpack.config.js` in the root of the Maven module, which you can then modify

In order to do this, modify the `webpack.config.js` in the root folder as follows:

```js
config.resolve.modules = ['/absolute/path/to/webcomponents/node_modules', 'node_modules'];
```
...
module.exports = merge(flowDefaults, {
  resolve:{
    modules: ['/absolute/path/to/webcomponents/node_modules', 'node_modules']
  }
});
```
